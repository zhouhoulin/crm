	...
	<script type="text/javascript">
        $(function(){
            ...
            $.post("${pageContext.request.contextPath }/baseDict_findByTypeCode.action",{"dictTypeCode":"002"},function(data){
                ...
                //使用EL获取值栈的数据
				$("#cust_source option[value='${model.baseDictSource.dictId}']").prop("selected","selected");
            },"json");
            $.post("${pageContext.request.contextPath }/baseDict_findByTypeCode.action",{"dictTypeCode":"006"},function(data){
                ...
                //使用EL获取值栈的数据
                $("#cust_level option[value='${model.baseDictLevel.dictId}']").prop("selected","selected");
            },"json");
            $.post("${pageContext.request.contextPath }/baseDict_findByTypeCode.action",{"dictTypeCode":"001"},function(data){
                ...
                //使用EL获取值栈的数据
                $("#cust_industry option[value='${model.baseDictIndustry.dictId}']").prop("selected","selected");
            },"json");
        });
	</script>
...
<s:form name="form1" id="form1" action="customer_update" namespace="/" method="POST" enctype="multipart/form-data" theme="simple">
	<s:hidden name="custId" value="%{model.custId}"/>
	<s:hidden name="custImage" value="%{model.custImage}"/>
			...
			<TD vAlign=top width="100%" bgColor=#ffffff>
				...
				<TABLE cellSpacing=0 cellPadding=5  border=0>
					...
					<TR>
						<td>客户名称：</td>
						<td>
							<s:textfield cssClass="textbox" cssStyle="WIDTH: 180px" maxlength="60"
										 name="custName" value="%{model.custName}"/>
						</td>
						<td>客户级别 ：</td>
						<td>
					...
					<TR>
						...
						<td>
							<s:textfield cssClass="textbox" cssStyle="WIDTH: 180px" maxlength="60"
										 name="custPhone" value="%{model.custPhone}"/>
						</td>

						<td>移动电话 ：</td>
						<td>
							<s:textfield cssClass="textbox" cssStyle="WIDTH: 180px" maxlength="60"
										 name="custMobile" value="%{model.custMobile}"/>
						</td>
